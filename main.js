// 괜찮아, 밥먹자 🏥💝 Master Nurse-Only Edition (Clean Result Version)

const FOOD_DATABASE = [
    { n: "엽기떡볶이", s: "엽기떡볶이", tags: ["spicy", "stress", "heavy"] },
    { n: "마라탕 & 꿔바로우", s: "마라탕", tags: ["spicy", "stress", "hungry"] },
    { n: "불닭발 & 주먹밥", s: "무뼈닭발", tags: ["spicy", "stress", "night"] },
    { n: "삼겹살 구이", s: "삼겹살", tags: ["meat", "exhausted", "heavy"] },
    { n: "한우 스테이크", s: "스테이크", tags: ["meat", "reward", "stable"] },
    { n: "영양 전복죽", s: "전복죽", tags: ["mild", "warm", "tired"] },
    { n: "뿌링클 치킨", s: "BHC 뿌링클", tags: ["fried", "reward", "stable"] },
    { n: "소고기 쌀국수", s: "쌀국수", tags: ["warm", "tired", "after_night"] },
    { n: "수제 치즈버거", s: "수제버거", tags: ["heavy", "hungry", "fried"] },
    { n: "바삭 등심 돈카츠", s: "돈까스", tags: ["stable", "crispy", "mild"] },
    { n: "곱창 전골", s: "곱창전골", tags: ["stress", "heavy", "warm"] },
    { n: "망고 빙수", s: "망고빙수", tags: ["angry", "cold", "reward"] }
];

const EXTRA_FOOD_NAMES = ["찜닭", "부대찌개", "김치찜", "냉모밀", "마제소바", "텐동", "나시고랭", "팟타이", "푸팟퐁커리", "타코", "라자냐", "에그인헬", "감바스", "뇨끼", "리조또", "휘낭시에", "크로플", "젤라또", "티라미수", "반미", "분짜", "탄두리치킨", "인도커리", "샥슈카", "파에야", "봉골레", "라구파스타", "잠봉뵈르", "지코바", "허니콤보", "고추바사삭", "가마로강정", "신전떡볶이", "청년다방", "응급실떡볶이", "배떡", "직화오돌뼈", "닭발", "염통꼬치", "순대", "튀김범벅", "물어묵", "매운오뎅", "붕어빵", "호떡", "소떡소떡", "멘보샤", "크림새우", "유린기", "깐풍기", "양장피", "마파두부", "짬뽕", "볶음밥", "딤섬", "고구마맛탕", "츄러스", "소르베", "말차빙수", "앙버터", "바게트", "소금빵", "도넛", "핫도그", "칠리독", "콘독", "어니언링", "해물파전", "김치전", "육전", "편육", "제육볶음", "오징어소면", "낙지볶음", "장어덮밥", "스테이크덮밥", "우츠동", "가츠동", "에비동", "카레라이스", "하이라이스", "오므라이스", "새우볶음밥", "잡채밥", "비빔냉면", "회냉면", "콩국수", "비빔국수", "잔치국수", "수제비", "칼국수", "모듬만두", "갈비탕", "곰탕", "설렁탕", "순대국", "내장탕", "뼈해장국", "감자탕", "추어탕", "육개장", "미역국", "소고기무국", "콩나물국밥", "선지국밥", "해장국", "닭개장", "삼계탕", "안동찜닭", "간장치킨", "양념치킨", "마늘치킨", "또봉이", "시장통닭", "오리주물럭", "훈제오리", "불고기", "갈비찜", "돼지갈비", "폭립", "치즈돈까스", "고구마돈까스", "차돌박이", "우삼겹", "대창", "막창", "곱창", "특양", "대창덮밥", "연어장덮밥", "간장새우장", "양념게장", "꼬막비빔밥", "물회", "조개구이", "방어회", "광어회", "우럭회", "도미회", "참치회", "육사시미", "산낙지", "낙곱새", "쭈꾸미볶음", "꼼장어", "장어구이"];
...
EXTRA_FOOD_NAMES.forEach((name, i) => {
    ...
    FOOD_DATABASE.push({ n: name, s: name, tags: [mainTag, "hungry"] });
});

const SUMMARIES = [
    "빌런 보호자 때문에 멘탈이 바스러진 당신, 강력한 수혈이 시급해요 😭",
...
const BRANCHES = {
    ...
    after_work: [
        { text: "오늘 스테이션 분위기는 어땠어?", options: [{t:"완전 스테이블(Stable)!", s:"stable"}, {t:"조금 바빴어", s:"mild"}, {t:"선배들 땜에 멘탈 나감", s:"stress"}, {t:"수쌤이 자꾸 불러서 피 말림", s:"stress"}, {t:"완전 헬파티 😭", s:"exhausted"}] },
        { text: "오늘 가장 힘들게 한 빌런은 누구야?", options: [{t:"말 안 듣는 환자/보호자", s:"angry"}, {t:"사고 치는 어리버리 신규", s:"stress"}, {t:"이해 안 가는 오더 내리는 의사", s:"stress"}, {t:"태움이나 다름없는 선배의 압박", s:"angry"}, {t:"쉴 틈 없는 응급 상황", s:"exhausted"}] },
        ...
        { text: "밥 먹을 여유는 좀 있었어?", options: [{t:"10시간 생공복... 배고파 죽음", s:"hungry"}, {t:"병원 밥 대충 넘김", s:"heavy"}, {t:"중간에 간식 좀 주워 먹음", s:"light"}, {t:"지쳐서 입맛조차 없어", s:"mild"}, {t:"전투적으로 먹긴 함", s:"stable"}] },
        ...
        { text: "식사하고 바로 잠들 예정이야?", options: [{t:"씻고 바로 기절", s:"mild"}, {t:"맥주 한 잔 하며 넷플릭스", s:"fried"}, {t:"잠 안 와서 술 생각남", s:"spicy"}, {t:"이제 막 밤을 지새워야 해", s:"energy"}, {t:"하소연하며 스트레스 풀래", s:"reward"}] },
    ...
    before_work: [
        { text: "무슨 근무 가?", options: [{t:"데이(Day)", s:"light"}, {t:"이브닝(Eve)", s:"energy"}, {t:"나이트(Night)", s:"heavy"}, {t:"기타 업무", s:"mild"}, {t:"불려가는 중", s:"angry"}] },
        { text: "오늘 멤버 확인했어?", options: [{t:"천사 조합!", s:"stable"}, {t:"무난무난", s:"mild"}, {t:"빌런 섞여있음", s:"spicy"}, {t:"내가 다 해야 함", s:"meat"}, {t:"무서운 선배 듀티", s:"stress"}] },
        { text: "솔직한 기분은?", options: [{t:"가기 싫어 눈물 남", s:"reward"}, {t:"돈 벌러 가자 해탈", s:"mild"}, {t:"제발 스테이블하길", s:"stable"}, {t:"카페인 수혈 시급", s:"energy"}, {t:"아무 생각 없음", s:"mild"}] },
        { text: "잠은 좀 푹 잤어?", options: [{t:"꿀잠 잤어!", s:"stable"}, {t:"병원 꿈 꾸다 설침", s:"stress"}, {t:"거의 못 잤어", s:"heavy"}, {t:"자꾸 깼어", s:"warm"}, {t:"자도 피곤해", s:"exhausted"}] },
        { text: "가장 걱정되는 게 뭐야?", options: [{t:"IV 실패 등땀", s:"stress"}, {t:"인계 털림", s:"angry"}, {t:"수쌤 잔소리", s:"stress"}, {t:"바빠서 굶음", s:"heavy"}, {t:"실수해서 혼남", s:"stress"}] },
        { text: "지금 입안 상태는?", options: [{t:"텁텁해서 상큼한 거", s:"light"}, {t:"텅 비어 고소한 거", s:"meat"}, {t:"매콤한 거", s:"spicy"}, {t:"속 쓰린 느낌", s:"warm"}, {t:"단 게 땡김", s:"reward"}] },
        { text: "오늘 예상 강도는?", options: [{t:"스테이블!", s:"stable"}, {t:"정신없을 예정", s:"heavy"}, {t:"이미 멘붕", s:"stress"}, {t:"평소만큼", s:"mild"}, {t:"이미 해탈", s:"stable"}] },
        { text: "근무 후 계획은?", options: [{t:"친구랑 맛집!", s:"reward"}, {t:"씻고 바로 기절", s:"mild"}, {t:"운동(갓생)", s:"energy"}, {t:"밀린 일/공부", s:"mild"}, {t:"보상 배달!", s:"heavy"}] },
        { text: "마지막으로 오늘 각오는?", options: [{t:"무사 퇴근하자!", s:"mild"}, {t:"환자 살리자!", s:"reward"}, {t:"난 할 수 있다!", s:"energy"}, {t:"사직서 참자", s:"stress"}, {t:"평화롭길!", s:"stable"}] }
    ],
    off_day: [
        { text: "오늘 오프 어떻게 시작했어?", options: [{t:"오후까지 기절", s:"reward"}, {t:"일찍 놀러 나감", s:"energy"}, {t:"밀린 일/공부", s:"stable"}, {t:"하루종일 뒹굴", s:"mild"}, {t:"대청소 완료", s:"heavy"}] },
        { text: "병원 냄새 빠진 것 같아?", options: [{t:"완전!", s:"stable"}, {t:"아니, 자꾸 단톡 알람", s:"angry"}, {t:"오프인데 꿈 꿈", s:"tired"}, {t:"인계 실수 생각", s:"stress"}, {t:"별 생각 없음", s:"stable"}] },
        { text: "어디가 제일 아파?", options: [{t:"종아리랑 발바닥", s:"warm"}, {t:"어깨랑 허리", s:"meat"}, {t:"눈이랑 머리", s:"mild"}, {t:"몸살 기운", s:"warm"}, {t:"안 아픔!", s:"stable"}] },
        { text: "행복 지수는?", options: [{t:"100% 완전 행복", s:"reward"}, {t:"80% 평화롭고 좋음", s:"stable"}, {t:"50% 무난무난", s:"mild"}, {t:"20% 내일 출근 걱정", s:"stress"}, {t:"0% 너무 아픔", s:"warm"}] },
        { text: "오늘 식사 컨셉은?", options: [{t:"화려한 폭식!", s:"heavy"}, {t:"분위기 외식", s:"reward"}, {t:"가볍게 관리", s:"light"}, {t:"편하게 배달", s:"mild"}, {t:"정갈한 집밥", s:"meat"}] },
        { text: "스트레스 풀렸어?", options: [{t:"응, 다 잊음!", s:"stable"}, {t:"어느 정도는", s:"mild"}, {t:"아직 조금 남아", s:"spicy"}, {t:"맛있는 거 먹어야 함", s:"heavy"}, {t:"오프가 더 길었으면", s:"reward"}] },
        { text: "칼로리 신경 써?", options: [{t:"오프인데? 노 신경!", s:"heavy"}, {t:"조금은?", s:"light"}, {t:"식단 중이야 😭", s:"light"}, {t:"맛있으면 0칼로리", s:"fried"}, {t:"관리!", s:"light"}] },
        { text: "지금 땡기는 식감은?", options: [{t:"바삭한 거", s:"fried"}, {t:"쫄깃한 고기", s:"meat"}, {t:"상큼한 거", s:"light"}, {t:"부드럽고 달콤", s:"reward"}, {t:"뜨끈한 국물", s:"warm"}] },
        { text: "오늘의 컨셉은?", options: [{t:"완벽한 보상!", s:"reward"}, {t:"내일을 위한 충전", s:"stable"}, {t:"그냥 아무것도 안 함", s:"mild"}, {t:"밀린 일 해결", s:"energy"}, {t:"사랑하는 사람과 함께", s:"reward"}] }
    ]
};
